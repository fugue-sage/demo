    ...
    if (env.BRANCH_NAME =~ /^feature\/.*$/) {
      withEnv(["ENVIRONMENT=DEV"]) {
        withCredentials([[$class: 'StringBinding', credentialsId: 'DEMO_AWS_ACCESS_KEY_ID', variable: 'AWS_ACCESS_KEY_ID'], 
          ...
          if(ret == 0) {
            sh("fugue update ${env.BRANCH_NAME} ControlsDemo.lw -y")
          } else {
            sh("fugue run ControlsDemo.lw -a ${env.BRANCH_NAME}")
          }
        }
      }
    }
    if (env.BRANCH_NAME =~ /^develop$/) {
      withEnv(["ENVIRONMENT=DEV"]) {
        withCredentials([[$class: 'StringBinding', credentialsId: 'DEMO_AWS_ACCESS_KEY_ID', variable: 'AWS_ACCESS_KEY_ID'],
          ...
          if(ret == 0) {
            sh('fugue update develop ControlsDemo.lw -y')
          } else {
            sh('fugue run ControlsDemo.lw -a develop')
          }
        }
      }
    }
    if (env.BRANCH_NAME =~ /^release\/.*$|^hotfix\/.*$/) {
      withEnv(["ENVIRONMENT=DEV"]) {
        withCredentials([[$class: 'StringBinding', credentialsId: 'DEMO_AWS_ACCESS_KEY_ID', variable: 'AWS_ACCESS_KEY_ID'], 
          ...
          if(ret == 0) {
            sh('fugue update staging ControlsDemo.lw -y')
          } else {
            sh('fugue run ControlsDemo.lw -a staging --account staging')
          }
        }
      }
    }
    if (env.BRANCH_NAME =~ /^master$/) {
      withEnv(["ENVIRONMENT=DEV"]) {
        withCredentials([[$class: 'StringBinding', credentialsId: 'DEMO_AWS_ACCESS_KEY_ID', variable: 'AWS_ACCESS_KEY_ID'], 
          ...
          if(ret == 0) {
            sh('fugue update production ControlsDemo.lw -y')
          } else {
            sh('fugue run ControlsDemo.lw -a production')
          }
        }
      }
    }
...
def run_dry_run() {
  withCredentials([[$class: 'StringBinding', credentialsId: 'FUGUE_ROOT_USER', variable: 'FUGUE_ROOT_USER']]) {
    ...
    //sh('fugue status')
    //sh('fugue run ControlsDemo.lw --dry-run')
  }
}